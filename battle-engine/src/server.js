/* eslint-disable no-control-regex, no-unused-vars, no-useless-assignment */
...
const redisConnectTimeoutMs = readPositiveIntEnv('REDIS_CONNECT_TIMEOUT_MS', 20000);
const redisCommandTimeoutMs = readPositiveIntEnv('REDIS_COMMAND_TIMEOUT_MS', 15000);
const redisTls = redisUrl?.startsWith('rediss://') ? { rejectUnauthorized: false } : false;
...
// ΓöÇΓöÇ Redis-backed helpers (fall back to in-memory when Redis is down) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function createBullConnection(name) {
  ...
  // Upstash (rediss://) requires explicit TLS ΓÇö IORedis does not infer it reliably
  if (redisTls) opts.tls = redisTls;
...
export async function createMatch(body) {
  ...
  try {
      const isSinglePlayer = newMatchId.startsWith('daily_') || roomType === 'PRACTICE';
      ...
      await measureMongoQuery(
        ...
        () => MatchModel.create({
          ...
          roomType,
          status: isSinglePlayer ? 'RACING' : 'WAITING',
          startTime: isSinglePlayer ? Date.now() : undefined,
          endTime: isSinglePlayer ? Date.now() + durationMs : undefined,
      ...
      if (isSinglePlayer) {
        await scheduleMatchExpiry(newMatchId, durationMs);
      }
  } catch (error) {
    if (error?.code === 11000) {
...
export async function queueCodeRun(action, body) {
...
export async function queueAnalysis(body) {
  const matchId = validateMatchId(body.matchId);
  const participant = resolveParticipant(body);
...
function computeNewBadges(player, context) {
  ...
  if (context.isFirstMatch) check('first_victory', 'First Victory', '≡ƒÅà');
  if (player.wins >= 10) check('10_wins', '10 Wins', '≡ƒÑë');
  if (player.wins >= 50) check('50_wins', '50 Wins', '≡ƒÑê');
  if (player.wins >= 100) check('100_wins', '100 Wins', '≡ƒÅå');
  if (player.currentStreak >= 3) check('streak_3', '3 Win Streak ≡ƒöÑ', '≡ƒöÑ');
  if (player.currentStreak >= 5) check('streak_5', '5 Win Streak ≡ƒöÑ', 'ΓÜí');
  if (player.currentStreak >= 10) check('streak_10', '10 Win Streak ≡ƒöÑ', '≡ƒÆÄ');
  if (context.durationMs && context.durationMs < 60000) check('speed_demon', 'Speed Demon', 'ΓÜí');
...
export async function getMatchById(matchId) {
  await connectDB();
  const game = await measureMongoQuery('match.lookup', () => MatchModel.findOne({ matchId }).lean());
  if (!game) throw new Error('Match not found');

  const problem = await measureMongoQuery('question.read', () => Question.findById(game.problemId).lean());
